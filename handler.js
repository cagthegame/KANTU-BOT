...
—◉ *Contacto para apelaciones:* wa.me/5215591478197
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await mconn.conn.reply(nk.from, `Hola *@${nk.from.split('@')[0]}*, las ${nk.isVideo ? 'videollamadas' : 'llamadas'} no están permitidas, serás bloqueado.\n-\nSi accidentalmente llamaste póngase en contacto con mi creador para que te desbloquee! (5215591478197) `, false, {mentions: [nk.from]});
        // let data = global.owner.filter(([id, isCreator]) => id && isCreator)
        // await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;CRXS MODS 👑;;;\nFN:CRXS MODS 👑\nORG:CRXS MODS 👑\nTITLE:\nitem1.TEL;waid=5215591478197:+521 559 147 8197\nitem1.X-ABLabel:CRXS MODS 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:CRXS MODS 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: 'CRXS MODS 👑', contacts: [{vcard}]}}, {quoted: callmsg});
